...
> - **Wave 2 #5** (`1e2ac4b`, report-only): `with_usages` saved-call value (offline, new `usages-ts` fixture + `expectedUsages` golden → `usageRecall`/`withUsagesCallsDelta`; measured 1.00 / +1.0) and a CI-excluded Voyage **reranker A/B** (`mrrAt1Delta`, gated on `CODESIFT_EVAL_RERANK=1`+`VOYAGE_API_KEY`). Surfaced in the summary only — never feeds `computeLosses`; `losses.json` unchanged. Implementation delegated to pi under a written spec, then reviewed + gate-verified. Paraphrase goldens (Task C) intentionally skipped to keep the gate safe.
> - **Wave 2 is complete** (#4, #6, #7 shipped; #5 report-only landed). Build env: Node 22 (`~/.nvm/versions/node/v22.22.2/bin`), gate = `pnpm run ci` (green, 0 new losses).
